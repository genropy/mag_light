...
class Table(object):
    def config_db(self, pkg):
        ...
        tbl.column('valore_unitario', dtype='money', name_long='!![it]Prezzo unitario', name_short='P.U')
        ...
        tbl.formulaColumn('giacenza_curr_dep', select=dict(table='mag_light.giacenza_prodotto_deposito', 
                                columns='$quantita_disponibile', where='$prodotto_id=#THIS.id AND $deposito_id=:env_current_deposito_id'),
        ...
        tbl.formulaColumn('giac_attesa_curr_dep', select=dict(table='mag_light.giacenza_prodotto_deposito', 
                                columns='$quantita_attesa', where='$prodotto_id=#THIS.id AND $deposito_id=:env_current_deposito_id'),
    ...
    #DP flag "materiale di consumo", se False diventa disponibile per comodato
    #DP codice EAN o altro produttore
