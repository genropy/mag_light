...
class Table(object):
    def config_db(self, pkg):
        ...
        tbl.column('deposito_id', size='22', group='_', name_long='!![it]Deposito').relation(
                        'deposito.id', relation_name='movimenti', mode='foreignkey', onDelete='raise')         
   ...
   #     tbl.formulaColumn('__protected_by_confermato', "($in_attesa IS NOT TRUE)", dtype='B')
    ...
    def setDefaultValues(self, record):
        ...
        record['deposito_id'] = record['deposito_id'] or self.db.currentEnv['current_deposito_id']
    ...
    def counter_protocollo(self,record=None):
        ...
        dep = self.db.table('mag_light.deposito').readColumns(where='$id=:dep_id', 
                                        dep_id=record['deposito_id'], columns='$codice')
    ...
    @public_method
    def creaTrasferimento(self, movimento_id=None, deposito_id=None, **kwargs):
        ...
        trasferimento = self.newrecord(movimento_id=movimento_id, 
                            movimento_tipo=movimento_tipo, deposito_id=deposito_id)
        ...
        for movimento_riga in movimenti_righe:
            ...
            trasferimento_riga = self.db.table('mag_light.movimento_riga').newrecord(prodotto_id=movimento_riga['prodotto_id'], 
                                ...
                                deposito_id=deposito_id, quantita_attesa= - (quantita))
